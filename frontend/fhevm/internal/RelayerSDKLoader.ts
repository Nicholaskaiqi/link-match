...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        console.log("[RelayerSDKLoader] script onerror... trying local fallback");
        const localScript = document.createElement("script");
        localScript.src = SDK_LOCAL_URL;
        localScript.type = "text/javascript";
        localScript.async = true;

        localScript.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            console.log("[RelayerSDKLoader] local fallback onload FAILED...");
            reject(
              new Error(
                `RelayerSDKLoader: Relayer SDK script loaded from ${SDK_LOCAL_URL}, but window.relayerSDK is invalid.`
              )
            );
            return;
          }
        ...
        localScript.onerror = () => {
          console.log("[RelayerSDKLoader] local fallback script onerror...");
          reject(
            new Error(
...
function isFhevmRelayerSDKType(
  ...
  const hasEthereumConfig =
    objHasProperty(o, "EthereumConfig", "object", trace) ||
    objHasProperty(o, "ZamaEthereumConfig", "object", trace);
  if (!hasSepoliaConfig && !hasEthereumConfig) {
    trace?.(
      "RelayerSDKLoader: relayerSDK config object is invalid (expected SepoliaConfig or EthereumConfig/ZamaEthereumConfig)."
    );
    return false;
  }
