...
export const createFhevmInstance = async (parameters: {
  ...
  const configSource =
    (relayerSDK as unknown as Record<string, unknown>)["ZamaEthereumConfig"] ||
    (relayerSDK as unknown as Record<string, unknown>)["EthereumConfig"] ||
    (relayerSDK as unknown as Record<string, unknown>)["SepoliaConfig"];

  if (!configSource || typeof configSource !== "object") {
    throw new Error(
      "Unable to resolve relayerSDK configuration (expected ZamaEthereumConfig/EthereumConfig/SepoliaConfig)."
    );
  }

  const aclAddress = (configSource as Record<string, unknown>)[
    "aclContractAddress"
  ] as string;
